...
async function processChapterText({
  ...
  // Debug: print all extracted paragraphs for inspection
  if (chapterId && (chapterId.toLowerCase().includes('chapter-1') || chapterId.toLowerCase().includes('chapter-2'))) {
    console.log(`Filtered paragraphs for ${chapterId}:`, paragraphs.map(p => p.content));
  }
