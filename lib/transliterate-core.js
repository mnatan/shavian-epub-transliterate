...
async function processChapterText({
  ...
  // Helper to normalize text for comparison (remove punctuation, whitespace, lowercase)
  ...
  // Book title for filtering (from chapterTitle or a known value)
  const bookTitle = 'Harry Potter and the Sorcerer\'s Stone';
  const normalizedBookTitle = normalizeText(bookTitle);

  // Filter out paragraphs that match the book title
  paragraphs = paragraphs.filter(p => normalizeText(p.content) !== normalizedBookTitle);
  ...
  // Skip splitting when --skip-original is used (includeOriginal = false)
  const splitLongParagraphs = (content) => {
    // When --skip-original is used, don't split paragraphs
    if (!includeOriginal) {
      return [content];
    }
  ...
  for (const block of blocks) {
    if (block.type === 'image') {
      ...
      // Clean up line breaks within words before processing
      let cleanedContent = block.content
        // Replace newlines followed by spaces with just a space (fix broken words)
        .replace(/\n\s+/g, ' ')
      ...
      let chunks = splitLongParagraphs(cleanedContent);

      // Merge quote-only chunks and short leftover chunks with the previous chunk
      const quoteOnlyRegex = /^\s*(["'""«»]|&quot;|&#39;)\s*$/;
      for (let i = 1; i < chunks.length; ) {
        if (quoteOnlyRegex.test(chunks[i]) || chunks[i].trim().length < 50) {
          chunks[i - 1] += chunks[i];
          chunks.splice(i, 1);
        } else {
          i++;
